...
# -*- coding: utf-8 -*-
"""
Generador de Gráficas para Portal Agro-comercial del Huila
Genera gráficas específicas basadas en el contenido del artículo aplicado
"""
...
def generar_comparacion_legacy_moderno():
    """Genera gráfica comparando tiempo invertido en sistemas legacy vs modernos"""
    ...
    bars1 = ax.bar(x - width, tiempo_desarrollo, width, label='Corrección de Fallos', color='#C73E1D')
    ...
    ax.set_title('Distribución del Tiempo de Desarrollo: Legacy vs Moderno\n(Portal Agro-comercial del Huila)')
    ax.set_xticks(x)
...
def generar_factores_exito():
    """Genera gráfica de factores predictivos de éxito en refactorización"""
    ...
    ax.set_title('Factores Predictivos de Éxito en Proyectos de Refactorización\n(Portal Agro-comercial del Huila)')
    ax.set_ylim(0, 5)
    ...
    ax.axhline(y=4, color='red', linestyle='--', alpha=0.7, label='Crítico (≥4.0)')
...
def generar_evolucion_metricas_portal():
    """Genera gráfica de evolución temporal de métricas del Portal"""
    ...
    # Gráfica 1: Cobertura de Pruebas
    ...
    ax1.set_title('Evolución de Métricas de Calidad del Portal Agro-comercial del Huila')
    ...
    # Gráfica 2: Defectos y Velocidad
    ax2_twin = ax2.twinx()
    
    line1 = ax2.plot(meses, defectos_produccion, 's-', color='#C73E1D', linewidth=3, markersize=8, label='Defectos en Producción')
    ...
    ax2.set_title('Defectos en Producción vs Velocidad del Equipo')
...
def generar_pipeline_devsecops():
    ...
    # Gráfica 2: Tasa de éxito por etapa
    tasas_exito = [95, 88, 99]  # porcentaje de éxito
...
def generar_comparacion_arquitecturas():
    """Genera comparación entre estrategias de modernización"""
    
    criterios = ['Riesgo', 'Tiempo', 'Calidad', 'Costo', 'Mantenibilidad']
    refactorizacion = [9, 7, 8, 6, 9]  # Puntuación 1-10
    ...
    bars1 = ax.bar(x - width/2, refactorizacion, width, label='Refactorización Incremental', 
    ...
    ax.set_title('Comparación de Estrategias de Modernización\n(Portal Agro-comercial del Huila)')
    ax.set_xticks(x)
...
def generar_roi_beneficios():
    """Genera gráfica de ROI y beneficios proyectados"""
    
    categorias = ['Reducción\nCostos Mantenimiento', 'Aumento\nVelocidad Desarrollo', 'Reducción\nDefectos', 'Mejora\nCalidad Código']
    ...
    ax.set_xlabel('Métricas de Impacto')
...
def main():
    """Función principal para generar todas las gráficas del Portal"""
